  ...
  return (
    <div className="relative w-full flex flex-col items-center">
      ...
      <div className="flex justify-between w-full max-w-[220px] mt-2 text-[10px] uppercase tracking-widest opacity-50">
        <span>
          {dates[0].split("-")[1]}/{dates[0].split("-")[0].slice(2)}
        </span>
        <span>
          {dates[dates.length - 1].split("-")[1]}/
          {dates[dates.length - 1].split("-")[0].slice(2)}
        </span>
      </div>
    </div>
  );
...
const StoryLayout = ({ bg, children }) => (
  <div
    className={`w-full h-full flex flex-col items-center justify-center p-6 ${bg} text-white text-center`}
  >
    {children}
  </div>
);
...
const getPieceInfo = (symbol) => {
  switch (symbol) {
    case "P":
      return {
        name: "El Peón",
        icon: <ChessPawn size={80} className="text-gray-300" />,
        desc: "Alma del ajedrez",
      };
    case "N":
      return {
        name: "El Caballo",
        icon: <ChessKnight size={80} className="text-orange-400" />,
        desc: "Maestro de la táctica",
      };
    case "B":
      return {
        name: "El Alfil",
        icon: <ChessBishop size={80} className="text-yellow-400" />,
        desc: "Francotirador",
      };
    case "R":
      return {
        name: "La Torre",
        icon: <ChessRook size={80} className="text-yellow-400" />,
        desc: "Fuerza bruta",
      };
    case "Q":
      return {
        name: "La Dama",
        icon: <ChessQueen size={80} className="text-yellow-400" />,
        desc: "Poder absoluto",
      };
    case "K":
      return {
        name: "El Rey",
        icon: <ChessKing size={80} className="text-yellow-400" />,
        desc: "El líder",
      };
    default:
      return { name: "Desconocido", icon: <BadgeQuestionMark />, desc: "" };
  }
};
...
const ChessWrapped = ({ player, playerData }) => {
  ...
  const data = useMemo(() => {
    ...
    if (total.personality === "Barry Allen")
      personalityIcon = (
        <Zap size={80} className="mb-6 text-yellow-400 animate-pulse" />
      );
    if (total.personality === "El Carnicero")
      personalityIcon = <Sword size={80} className="mb-6 text-red-500" />;
    if (total.personality === "El Estratega")
      personalityIcon = <Brain size={80} className="mb-6 text-blue-300" />;
        ...
        <StoryLayout bg="bg-gradient-to-b from-gray-900 via-gray-800 to-black">
          {player.avatar && (
            <img
              src={player.avatar}
              alt="Avatar"
              className="w-32 h-32 rounded-full border-4 border-green-500 mb-6 shadow-2xl"
            />
          )}
    ...
    // 2. TIEMPO
    {
      content: () => (
        <StoryLayout bg="bg-gradient-to-br from-slate-800 to-black">
          ...
          <p className="text-2xl font-medium mb-2 text-blue-200">
            Horas Jugadas
          </p>
        ...
        <StoryLayout bg="bg-gradient-to-br from-indigo-900 to-purple-900">
          ...
          <div className="bg-white/10 p-6 rounded-xl w-full max-w-sm mb-4 border border-white/5">
            ...
            <h2 className="text-4xl font-bold">
              {data.total.dia_semana_top.name}
            </h2>
          </div>
    ...
    storiesList.push({
      content: () => (
        <StoryLayout bg={`bg-gradient-to-br ${cat.gradient}`}>
          {cat.icon}
          <h2 className="text-5xl font-black uppercase tracking-wider mb-1">
            {cat.label}
          </h2>
    ...
    data.piezaFavorita.count > 0 && {
      content: () => (
        <StoryLayout bg="bg-gradient-to-br from-indigo-900 via-purple-900 to-black">
          <div className="animate-bounce mb-6">{data.piezaFavorita.icon}</div>

          <p className="text-xl opacity-70 mb-2 uppercase tracking-widest">
            Tu Fiel Compañero
          </p>
          ...
          <div className="grid grid-cols-2 gap-4 w-full max-w-xs text-left">
            ...
            <div className="bg-black/30 p-4 rounded-xl border border-white/10">
              ...
              <p className="text-3xl font-bold text-pink-300">
                {(
                  (data.piezaFavorita.count / data.piezaFavorita.totalMoves) *
                  100
                ).toFixed(0)}
                %
              </p>
          ...
          {data.total.nemesis.count > 0 && (
            <div className="mb-6 w-full">
              ...
              <p className="text-sm text-red-200">
                Te ganó {data.total.nemesis.count} veces
              </p>
            </div>
          )}
          {data.total.nemesis.count > 0 && data.total.pet.count > 0 && (
          ...
          {data.total.pet.count > 0 && (
            <div className="mt-6 w-full">
              ...
              <p className="text-sm text-green-200">
                Le ganaste {data.total.pet.count} veces
              </p>
            </div>
          )}
        </StoryLayout>
        ...
        <StoryLayout bg="bg-gradient-to-br from-cyan-600 to-blue-800">
          ...
          <div className="w-full max-w-xs space-y-4">
            {data.topFriends.map((friend, idx) => (
              <div
                key={idx}
                className="flex justify-between items-center bg-black/20 p-4 rounded-lg border border-white/10"
              >
