...
const ChessWrapped = ({ player, playerData }) => {
  ...
  console.log("Stats recibidas en ChessWrapped:", stats);

  const getWrappedData = () => {
    const modes = ['bullet', 'blitz', 'rapid', 'daily'];
    let totalGames = 0, totalWins = 0, bestRating = 0, bestMode = 'N/A', nombreBestMode = '',nemesis={},pet={},nemesis_final=[],pet_final=[];
    modes.forEach(mode => {
        if (stats[mode]) {
            console.log(stats[mode]);
            const wins = stats[mode].w_with + stats[mode].w_black|| 0;
            
            totalGames += stats[mode].cantidad_games || 0;
            totalWins += wins;
            nemesis[stats[mode].nemesis] = (nemesis[stats[mode].nemesis] || 0) + 1;
            pet[stats[mode].pet] = (pet[stats[mode].pet] || 0) + 1

            if (stats[mode].elo_max > bestRating) {
                bestRating = stats[mode].elo_max;
                bestMode = mode.replace('chess_', '');
                nombreBestMode = mode
            }
        }
    });
    const { claveMax, valorMax } = Object.entries(nemesis).reduce((acumulador, [clave, valor]) => {
    return valor > acumulador.valorMax ? { claveMax: clave, valorMax: valor } : acumulador;
    }, { claveMax: '', valorMax: -Infinity });
    nemesis_final = { name: claveMax, count: valorMax };
    const { claveMax: claveMaxPet, valorMax: valorMaxPet } = Object.entries(pet).reduce((acumulador, [clave, valor]) => {
    return valor > acumulador.valorMax ? { claveMax: clave, valorMax: valor } : acumulador;
    }, { claveMax: '', valorMax: -Infinity });
    pet_final = { name: claveMaxPet, count: valorMaxPet };
  ...
  console.log("Datos de Chess Wrapped:", data);
  ...
  const stories = [
    {
      content: () => (
        <StoryLayout bg="bg-gradient-to-br from-gray-900 to-black">
          {player.avatar && (
              <img src={player.avatar} alt="Avatar" className="w-40 h-40 rounded-full border-4 border-green-500 mb-6 shadow-xl" />
          )}
          <h1 className="text-6xl font-bold mb-2">{player.username}</h1>
          <h2 className="text-2xl font-bold text-green-400 mb-6">CHESS WRAPPED</h2>
          <p className="text-lg text-gray-400">Disfruta de tu resumen del año 2025</p>
        </StoryLayout>
      ),
    },
    {
